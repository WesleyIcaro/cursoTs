...
const verifySenhaInput = document.querySelector('.password2');
form.addEventListener('submit', function (e) {
    ...
    if (!verifyUser(usuarioInput.value, emailInput.value, senhaInput.value, verifySenhaInput.value))
        return;
...
//   return send;
// }
const verifyUser = (username, email, senha, verifySenha) => {
    let validation = true;
    if (username.length < 3 || username.length > 255) {
        showErrorMessage(usuarioInput, 'O Campo nome deve ter entre 3 e 255 caracteres');
        validation = false;
    }
